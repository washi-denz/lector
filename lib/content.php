<?php
	...
	class content
	{
		...
		public function get_js_top(){
			return (isset($this->code_secret["url_script_top"])) ? $this->get_secret("url_script_top"):"";
		}

		public function add_js_top($url){
			if(isset($this->code_secret["url_script_top"])){
				$this->code_secret["url_script_top"].='<script src="'.$url.'"></script>'."\n";
			}
			else{
				$this->code_secret["url_script_top"]='<script src="'.$url.'"></script>'."\n";
			}
		...
		public function add_attr($attr){
			if(isset($attr)){
				$this->code_secret["html_attr"]=$attr;
			}
		}
		public function get_attr(){
			return (isset($this->code_secret["html_attr"])) ? $this->get_secret("html_attr"):"";
		}
		// add clase a body
		public function add_class($name,$strClass=null){
			//strClass = 'text-gray-100 bg-gray-50'
			$this->code_secret[$name] = $strClass;
		}
		public function get_class($name){
			return (isset($this->code_secret[$name]))? $this->code_secret[$name]:null;
		}
	}
?>
